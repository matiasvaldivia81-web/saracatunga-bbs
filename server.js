...
function initSchema() {
  ...
  // Migraciones seguras - verifica columna antes de agregar
  ...
  if (!hasColumn('reports','resolved_by')) db.run("ALTER TABLE reports ADD COLUMN resolved_by INTEGER");
  if (!hasColumn('reports','resolved_at')) db.run("ALTER TABLE reports ADD COLUMN resolved_at TEXT");
